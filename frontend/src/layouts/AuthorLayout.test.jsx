...
vi.mock('../services/authService', () => ({
  default: {
    getCurrentUser: () => ({ email: 'author@test.com' }),
    logout: vi.fn().mockResolvedValue({}),
  },
}))

const renderAuthorLayout = () => {
  return render(
    <BrowserRouter>
      <AuthorLayout />
    </BrowserRouter>
...
describe('AuthorLayout Mobile Sidebar', () => {
  beforeEach(() => {
  ...
  it('hides desktop sidebar and shows mobile header at ≤1024px', () => {
    ...
    expect(screen.getByText('The Essence - Author')).toBeInTheDocument()
  ...
  it('shows overlay when sidebar is open', () => {
    Object.defineProperty(window, 'innerWidth', { writable: true, configurable: true, value: 768 })
    const { container } = renderAuthorLayout()
  ...
  it('uses brand color #10B981 for sidebar', () => {
    Object.defineProperty(window, 'innerWidth', { writable: true, configurable: true, value: 1280 })
    const { container } = renderAuthorLayout()

    // Check the logo icon uses the brand color
    const logoIcon = container.querySelector('[style*="background-color: rgb(16, 185, 129)"]') ||
                     container.querySelector('[style*="#10B981"]')
  ...
  it('restores body overflow on unmount', () => {
    Object.defineProperty(window, 'innerWidth', { writable: true, configurable: true, value: 768 })
    const { unmount } = renderAuthorLayout()
